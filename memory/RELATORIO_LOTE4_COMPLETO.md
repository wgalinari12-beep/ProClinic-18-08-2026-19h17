...
Nota técnica: o `REACT_APP_BACKEND_URL` correto do ambiente é `https://proclinic-app.preview.emergentagent.com` (o `.env` foi atualizado pela plataforma). Frontend e backend compartilham o mesmo host em produção, portanto não há problema de CORS no uso real.
